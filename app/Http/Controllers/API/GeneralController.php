...
class GeneralController extends Controller
{
    ...
    public function getGeneralSettings()
    {
        ...
        $settings = [
            ...
            'selectedHotel' => $query->where('key', 'selected_hotel')->first()->value ?? '',
            'bookingPrefix' => $query->where('key', 'booking_prefix')->first()->value ?? '',
            'gstPrefix' => $query->where('key', 'gst_number')->first()->value ?? '',
        ];
    ...
    public function updateGeneralSettings(Request $request)
    {
        $validator = $request->validate([
            ...
            'defaultAccount' => 'required',
            'bookingPrefix' => 'required|string|min:2|max:10',
            'gstPrefix' => 'required|string|min:5|max:15',
        ...
        $allSettings->where('key', 'booking_prefix')->first()->update(['value' => clean($request->bookingPrefix)]); 
        $allSettings->where('key', 'gst_number')->first()->update(['value' => clean($request->gstPrefix)]);   
