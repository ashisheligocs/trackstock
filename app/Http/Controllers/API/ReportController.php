...
class ReportController extends Controller
{
    ...
    public function balanceSheet()
    {
        ...
        //all transaction
        $types = ['ACCOUNT-PAYABLE', 'ACCOUNT-RECEIVABLE', 'GST-INPUT', 'GST-OUTPUT', 'OPERATING_REVENUE', 'EQUITY', 'INVENTORY', 'CHARGES'];
        $response = [];
        foreach ($types as $type) {
            $response[$type] = Account::where('status', 1)->where('slug', $type)->get()->sum('available_balance');
        }
        // inventory value
...
//        $invoiceDue = $totalSales - $invoiceTotalPaid;
        ...
        // bank balance
        $bankBalance = Account::where('status', 1)->whereHas('ledgerAccount', function ($q) {
            $q->where('is_bank', 1);
        })->get()->sum('available_balance');

        $inventoryValue = floatval($response['INVENTORY'] ?? 0);
        $revenue = floatval($response['OPERATING_REVENUE'] ?? 0);
        $AcPayable = floatval($response['ACCOUNT-PAYABLE'] ?? 0);
        $AcReceivable = floatval($response['ACCOUNT-RECEIVABLE'] ?? 0);
        $gstInput = floatval($response['GST-INPUT'] ?? 0);
        $gstOutput = floatval($response['GST-OUTPUT'] ?? 0);
        $charges = floatval($response['CHARGES'] ?? 0);
        ...
        $AcReceivable = $AcReceivable + $nonInvoiceDue;
...
//        $supplierTotalDue = $purchaseDue + $nonPurchaseDue;
        ...
        $AcPayable = $AcPayable + $nonPurchaseDue;

        // loan due
        $loanDue = LoanAuthority::where('status', 1)->get()->sum('due');

//        $buisnessTotal = $assets + $inventoryValue + $clientTotalDue + $bankBalance;
        $liabilities = $loanDue + $AcPayable + $gstOutput - $gstInput + $revenue + $charges;
        $totalAsset = $assets + $bankBalance + $inventoryValue + $AcReceivable;
        ...
        return [
            ...
            'outputGst' => round($gstOutput, 2),
            'inputGst' => round($gstInput, 2),
            'accPayable' => round($AcPayable, 2),
            'accReceivable' => round($AcReceivable, 2),
            'revenue' => round($revenue + $charges, 2),
