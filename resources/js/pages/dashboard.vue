<template>
  <div>
    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div v-if="$can('payment-sent-vs-payment-received')" class="col-md-6 col-xl-6">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">
                {{ $t("dashboard.profitLoss") }}
                <a href="#" class="badge badge-info " v-tooltip="$t('dashboard.profitLoss')">
        ...
        <div v-if="$can('top-clients')" class="col-md-6" :class="$can('payment-sent-vs-payment-received') ? 'col-xl-6' : 'col-lg-12'
      ">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">
                {{ $t("dashboard.salePurchase") }}
                <a href="#" class="badge badge-info " v-tooltip="$t('dashboard.salePurchase')">
                  <i class="fas fa-info"></i>
                </a>
              </h3>
            </div>
            <div class="card-body payment_chart">
              <template>
                <apexchart height="400"  type="bar"  :options="chartOptions1" :series="series1"></apexchart>
              </template>
            </div>
          </div>
        </div>
      ...
      <!-- <div v-if="$can('payment-sent-vs-payment-received') || $can('top-clients')" class="row">
        ...
        <div v-if="$can('top-clients')" class="col-md-12" :class="$can('payment-sent-vs-payment-received') ? 'col-xl-4' : 'col-lg-12'
          ">
          <TopClients />
        </div>
      </div> -->
      ...
      <!-- <div class="col-md-6" v-show="rooms_availability && rooms_availability.length">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title mt-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
          ...
          <div class="card-body">
                <div class="card">
                  <div class="table-responsive table-custom">
                    ...
                    <tbody>
                      <tr v-for="(data, i) in rooms_availability" :key="i">
                        <td>
                          <span>{{ i + 1 }}</span>
                        </td>
                        <td>{{ data.cat_name }}</td>
                        ...
                        <td>{{ data.total_confirm }}</td>
                        <td>{{ data.total_hold }}</td>
                      </tr>
...
export default {
  ...
  methods: {
    ...
    //       hotel = this.selectedHotel
    //     }
    //     let { data } = await axios.get(window.location.origin + `/api/room/list-with-status?&category=${category}&hotel=${hotel}`)
