            ...
            <div class="col-xl-8 col-8 float-right text-right">
              <div class="btn-group c-w-100">
                ...
                <a v-if="$can('purchase-edit') ||
                  $can('purchase-view') ||
                  $can('purchase-delete')
                  " href="/purchases/excel" v-tooltip="$t('common.export_table')" class="btn btn-info">
              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <th class="text-right">{{ inr }} {{ $t("common.subtotal") }}</th>
                    <!-- <th>{{ inr }} {{ $t("common.transport") }}</th> -->
                    <th>{{ inr }} {{ $t("common.discount") }}</th>
                    <th>{{ inr }} GST</th>
                    <th>{{ inr }} {{ $t("common.net_total") }}</th>
                    <th>{{ inr }} {{ $t("common.total_paid") }}</th>
                    <th>{{ inr }} {{ $t("common.total_due") }}</th>
                    ...
                    <td v-if="$can('purchase-edit') ||
                      ...
                      <div class="btn-group">
                        <a v-if="$can('purchase-view') && data.due > 0" v-tooltip="$t('common.add_payment')"
                          class="btn btn-info btn-sm" @click="handleModal(data)">
    ...
    <Modal v-if="showModal" @close="showModal = false">
      ...
      <div slot="body" class="row">
        <form role="form" @submit.prevent="savePayment" @keydown="form.onKeydown($event)" class="w-100">
          ...
          <div class="form-group col-12  flex-wrap d-none">
            <div class="pr-5">
