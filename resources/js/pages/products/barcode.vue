        ...
        <div class="card no-print">
          ...
          <form role="form" @submit.prevent="generateBarcode" @keydown="form.onKeydown($event)">
            <div class="card-body">
              ...
              <div class="row mt-3 mb-2">
                <div class="table-responsive table-custom w-95 m-auto">
                  <table class="table">
                    ...
                    <tbody v-if="form.product">
                      <tr>
                        <td>
                          {{ form.product.code | withPrefix(productPrefix) }}
                        </td>
              ...
              <div class="row">
                <div v-if="items" class="form-group col-md-6">
                  <input type="checkbox" v-model="generateWithPrice"/> {{ $t("Generate barcode with price") }}
                </div>
              </div>
            </div>
            <!-- /.card-body -->
          ...
          <div class="barcode-wrapper card-body" id="printMe">
            <div class="print-page-layout display-grid" :class="perPage" v-for="(pageNumber, i) in pages" :key="i">
              <div class="barcode-item" v-for="j in parseInt(pageNumber)" :key="j">
                ...
                <span v-if="generateWithPrice" class="barcode-name"> {{ $t("Price") }}: {{appInfo.currency.symbol}}{{ form.product.sellingPrice }} <br>
                  <span class="barcode-tax" v-if="form.product.taxRate>0">( {{ form.product.taxRate }}% {{$t("TAX")}} {{ form.product.taxType }} )</span> </span>
              </div>
            </div>
          </div>
        </div>
