                    ...
                    <div class="card-header setings-header">
                        ...
                        <div class="col-xl-8 col-8 float-right text-right">
                            <div class="btn-group c-w-100">
                                ...
                                <a href="/invoices/excel" v-tooltip="$t('common.export_table')" class="btn btn-dark">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                                        ...
                                        <td class="text-left">{{ data.client }}</td>
                                        <td>{{ (data.subTotal + (data.bookingDiscount || 0)) }}</td>
                                        <!-- <td>{{ data.transport }}</td> -->
                                        <!--
                                        <td>{{ data.client }}</td>
                                        <td>{{ (data.subTotal + (data.bookingDiscount || 0)) | forBalanceSheetCurrencyDecimalOnly}}</td>
                                        <td>{{ data.transport | forBalanceSheetCurrencyDecimalOnly }}</td>
 -->
                                        <td>
                                            {{ (data.bookingDiscount || data.discount) }}
                                            <span v-if="data.discountType == 1">({{ data.discountPercentage }}%)</span>
                                        </td>
                                        <td>{{ data.tax | forBalanceSheetCurrencyDecimalOnly }}</td>
                                        ...
                                        <td v-if="$can('invoice-view') ||
                                            ...
                                            " class="text-right no-print">
                                            <div class="btn-group">
                                                <a v-if="$can('invoice-view') && data.due && data.due.toFixed(1) > 0"
                                                    v-tooltip="$t('common.add_payment')" class="btn btn-info btn-sm"
                                                    @click="handleModal(data)">
                                                ...
                                                <router-link
                                                    v-if="$can('invoice-edit') && !data.booking && !data.isRestaurantOrder"
                                                ...
                                                <a v-if="$can('invoice-delete') && data.totalPaid == 0"
                                                    v-tooltip="$t('common.delete')" class="btn btn-danger btn-sm"
                                                    @click="deleteData(data.slug)">
                                                    <i class="fas fa-trash" />
                                                </a>
                                                <!-- <a v-if="data.totalPaid == 0"
                                                    v-tooltip="$t('common.cancel')" class="btn btn-danger btn-sm"
                                                    @click="cancelInvoice(data.slug)">
                                                    <i class="fas fa-ban" />
                                                </a> -->
                                            </div>
                                        </td>
                    ...
                    <small class="ml-2">
                        <i style="padding-left: 13px;">
                            All prices are in {{ '' | defaultwithCurrency }}
                        </i>
                    </small>
                    <div class="card-footer">
                        <div class="dtable-footer" v-if="shouldShowPerPageSelection">
        ...
        <Modal v-if="showModal">
            <h5 slot="header">
                <p>
                    ...
                    {{ form.selectedInvoice.invoiceNo }}
                </p>
            ...
            <div slot="body" class="row">
                <form role="form" @submit.prevent="savePayment" @keydown="form.onKeydown($event)" class="w-100">
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" readonly
                                v-model="form.selectedInvoice.invoiceTotal.toFixed(2)" />
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" readonly
                                v-model="form.selectedInvoice.due.toFixed(2)" />
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-md-6">
                    ...
                    <div class="form-group col-12 d-none flex-wrap">
                        <div class="pr-5">
            ...
            <template #modal-footer>
                <div class="mt-1">
                    ...
                    <router-link v-if="selectedClientId != ''"
                        :to="{ name: 'invoicePayments.create', params: { slug: selectedClientId }, }"
                        class="modal-default-button btn btn-success">Add Multiple Payment?</router-link>
...
export default {
    ...
    methods: {
        ...
        //         text: 'Are you sure you want to cancel Invoice',
        //         type: "warning",
