...
<!--                  {{ allData.client.companyName }}<br /></span>-->
          ...
          <div class="row position-relative mt-4 mb-4">
            ...
            <div class="col-12">
              <strong v-if="returnProducts.length" class="mt-3">{{ $t('common.return_products') }}:</strong>
              <strong v-if="returnService.length" class="mt-3">Invoice Services:</strong>
              <div v-if="allData.invoice" class="table-custom table-responsive text-center">
                <table class="table table-sm" v-if="returnProducts.length">
                  <thead>
                    <tr>
                      <th>{{ $t('common.s_no') }}</th>
                      <th>{{ $t('common.code') }}</th>
                  ...
                  <tbody v-if="returnProducts.length">
                    <tr v-for="(data, i) in returnProducts" :key="i">
                      ...
                      <td>
                        {{ (data.purchasePrice * data.invoiceQty) | withCurrency }}
                      </td>
                      <td class="text-right">
                ...
                <table class="table table-sm" v-if="returnService.length">
                  <thead>
                    <tr>
                      <th>{{ $t('common.s_no') }}</th>
                  ...
                  <tbody v-if="returnService.length">
                    <tr v-for="(data, i) in returnService" :key="i">
                      <td>{{ ++i }}</td>
                      <td>{{ data.serviceName }}</td>
                      <td>{{ data.invoiceQty }} </td>
...
export default {
  ...
  methods: {
    ...
    async getInvoiceReturn() {
      ...
      this.returnService = this.allData.invoiceReturnService
    ...
    calculateTotalAmount() {
      ...
      if (this.allData.invoiceReturnProducts) {
        invoiceSubTotal = this.allData.invoiceReturnProducts.reduce(function (
          ...
          return prev + Number(next.invoiceQty) * Number(next.purchasePrice)
        },
          0)
          console.log(this.allData.invoiceReturnProducts);
      ...
      // console.log('serviceData',this.allData.invoiceReturnService)
      if (this.allData.invoiceReturnService.length !== 0) {
        invoiceSubTotal = this.allData.invoiceReturnService.reduce(function (
          prev,
          next
        ) {
          ...
          0)
        invoiceReturn = this.allData.invoiceReturnService.reduce(function (
          prev,
          next
        ) {
