        ...
        <div class="card">
          ...
          <form role="form" @submit.prevent="saveInvoiceReturn" @keydown="form.onKeydown($event)">
            <div class="card-body">
                  ...
                  <table class="table table-hover table-sm text-center">
                    ...
                    <tbody>
                      <tr v-for="(item, i) in form.selectedProducts" :key="i">
                        ...
                        <td>
                          <div class="input-group custom-qty-input">
                            ...
                            <input type="number" step="any" :id="`returnQty-${i}`" :value="item.returnQty"
                              name="quantity" class="quantity-field border-0 incrementor" min="0" :max="item.maxQty"
                        ...
                        <td>{{ item.unitPrice | withCurrency }}</td>
                        <td>{{ item.productTax | withCurrency }}</td>
              ...
              <div v-if="form.selectedServices && form.selectedServices.length > 0" class="row mt-3 mb-4">
                <div v-if="
...
export default {
  ...
  methods: {
      ...
      if (this.form.client) {
        axios
          ...
          .then((response) => {
            ...
            console.log('client',this.clientInvoices);
          })
      } else {
        this.form.product = ''
        this.form.client = ''
    ...
    storeProducts() {
      ...
      this.form.invoiceTaxRate = this.form.invoice.product_tax
      ...
      if(this.form.invoice.invoiceProducts.length > 0){
        for (var key in this.form.invoice.invoiceProducts) {
          ...
          this.form.selectedProducts.unshift({
            ...
            taxRate: invoiceItem.product_tax,
            ...
            totalPrice: invoiceItem.purchasePricetotal,
            returnTotal: 0,
            productTax: invoiceItem.unitTax,
            totalTax: invoiceItem.taxTotal,
            // totalTax: invoiceItem.productTax,
            maxQty: invoiceItem.quantity - 1,
          })
        }
      }

      if(this.form.invoice.invoiceServices.length > 0){

        // this.form.selectedServices = this.form.invoice.invoiceServices;
        for (var key in this.form.invoice.invoiceServices) {
          let invoiceItem = this.form.invoice.invoiceServices[key]
          
          this.form.selectedServices.unshift({
              id: invoiceItem.service_id,
              service_name: invoiceItem.service_name,
              quantity: invoiceItem.quantity,
              price: invoiceItem.price,
              returnQty: invoiceItem.returnQty ?? 0,
              totalReturnQty: invoiceItem.quantity - invoiceItem.returnQty ?? 0,
              returnTotal: 0,
      ...
      if(this.form.selectedProducts.length > 0){
        ...
        if (selectedProduct && value >= 0 && value < selectedProduct.qty) {
          ...
          selectedProduct.returnTotal =
            selectedProduct.returnQty * selectedProduct.unitPrice
    ...
    calculateSum() {
      ...
      if(this.form.selectedProducts.length > 0){
        ...
        // console.log('invoiceTotal',this.form.invoiceTotal);
      ...
      if(this.form.selectedServices.length > 0){
        ...
        this.form.newTax = Number(
          (((this.form.invoice.invoiceServiceTax[0].rate * 2) / 100) * this.form.newSubTotal).toFixed(
            2
          )
        )
