      ...
      <div class="col-lg-12 col-xl-12">
        <div class="card">
          <div class="card-header">
            ...
            <router-link :to="{ name: 'invoiceReturns.index' }" class="btn btn-secondary float-right">
          ...
          <form role="form" @submit.prevent="updateInvoiceReturn" @keydown="form.onKeydown($event)">
            <div class="card-body">
              ...
              <div v-if="(form.selectedProducts && form.selectedProducts.length > 0 || form.selectedServices && form.selectedServices.length > 0)" class="row mt-3 mb-2">
                <div v-if="
                  form.errors.errors && form.errors.errors.selectedProducts
                " class="w-95 m-auto">
                ...
                <div v-if="
                  form.errors.errors && form.errors.errors.selectedServices
                " class="w-95 m-auto">
                  <div v-for="(msg, i) in form.errors.errors.selectedServices" :key="i" class="callout callout-danger">
                    <p><i class="icon fas fa-ban"></i> {{ msg }}</p>
                  </div>
                </div>

                <div class="table-responsive table-custom w-95 m-auto" v-if="form.selectedProducts.length > 0">
                ...
                <div class="table-responsive table-custom w-95 m-auto" v-if="form.selectedServices.length > 0">
                  <table class="table table-hover table-sm text-center">
                    <thead>
                      <tr>
                        <th>{{ $t("common.s_no") }}</th>
                    ...
                    <tbody>
                      <tr v-for="(item, i) in form.selectedServices" :key="i">
                        <td>{{ ++i }}</td>
                        ...
                        <td>
                          <div class="input-group custom-qty-input">
                            <input type="button" value="-" class="button-minus icon-shape icon-sm btn-danger"
                              data-field="quantity" @click="updateItem(item.returnQty - 1, i - 1)" />
                            <input type="number" step="any" :id="`returnQty-${i}`" :value="item.returnQty"
                              name="quantity" class="quantity-field border-0 incrementor" min="0" :max="item.quantity"
                              @change="updateItem($event.target.value, i - 1)"
                              @keyup="updateItem($event.target.value, i - 1)" placeholder="Return Qty" />
                            <input type="button" value="+" class="button-plus icon-shape icon-sm btn-primary"
              ...
              <div v-if="form.invoice" class="row">
                ...
                <div class="form-group" :class="form.discountPercentage > 0 ? 'col-md-2' : 'col-md-4'">
                  ...
                  <input id="invoiceDiscount" v-model="form.invoiceDiscount" type="number" step="any"
                    class="form-control" name="invoiceDiscount" readonly />
                </div>
...
export default {
  ...
  methods: {
    ...
    async getInvoiceReturn() {
      ...
      this.form.selectedServices = this.assignServices(
        data.data.invoiceReturnService,data.data.invoiceReturnServiceTax
      ) 
      this.serviceTax = data.data.invoiceReturnServiceTax;
    },
    // assign products
    ...
    assignServices(service,serviceTax){
      this.form.selectedServices = [];
      for (var key in service) {
          let invoiceItem = service[key]
          
          this.form.selectedServices.unshift({
              id: invoiceItem.serviceID,
              service_name: invoiceItem.serviceName,
              quantity: invoiceItem.invoiceQty,
              price: invoiceItem.salePrice,
              returnQty: invoiceItem.returnQty ?? 0,
              totalReturnQty: invoiceItem.invoiceQty - invoiceItem.returnQty ?? 0,
              returnTotal: invoiceItem.salePrice * invoiceItem.returnQty,
              totalPrice: invoiceItem.salePrice * invoiceItem.invoiceQty
           });
        }
      this.calculateSum(serviceTax)
      return this.form.selectedServices
    },
    // updateItems
    updateItem(value, index) {
      
      if(typeof this.form.selectedProduct !== 'undefined'){
        if(this.form.selectedProduct.length > 0){
      ...
      if(typeof this.form.selectedServices !== 'undefined'){
        
          if(this.form.selectedServices.length > 0){
          let selectedService = this.form.selectedServices[index];
          
          if(selectedService && value >= 0 && value < selectedService.quantity){
            selectedService.returnQty = Number(value)
            selectedService.returnTotal =
            selectedService.returnQty * selectedService.price
            this.form.selectedServices[index] = selectedService
          }
          
          this.calculateSum(this.serviceTax);
        }
      }

    },
    // calculate sum
    calculateSum(serviceTax) {
      // calculate total
      ...
      this.form.invoiceTotal = Number(
        (
          ...
          this.form.invoiceDiscount
            ...
            //     tax.taxRate.forEach(taxRate => {
            //         if(taxRate.amount != 0){
            //             this.form.totalTax += (tax.unitPrice * (tax.qty- tax.returnQty) * taxRate.rate) / 100;
            //         }
            //     });
            // });

            // this.form.newTax = Number(this.form.totalTax).toFixed(2);
      ...
      if(this.form.selectedServices.length > 0){
        let length = this.form.selectedServices.length
        
        for (let i = 0; i < length; i++) {
          let looProduct = this.form.selectedServices[i]
          this.form.newSubTotal += Number(
            (
              (looProduct.quantity - looProduct.returnQty) *
              looProduct.price
            ).toFixed(2)
          )
          this.form.totalReturn += Number(looProduct.returnTotal.toFixed(2))
        ...
        this.form.newTax = Number(
          (((serviceTax[0].rate * 2) / 100) * this.form.newSubTotal).toFixed(
            2
          )
        )
