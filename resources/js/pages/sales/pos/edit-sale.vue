                                    ...
                                    <div class="relative">
                                        <div class="pos-box-img">
                                            <div v-if="product?.image">
                                                <img
                                                        class="pos-box-icon"
                                                        :src="product?.image?.replace('public/', '/storage/')"
                                                        alt="product image"
                                                />
                                            </div>
                ...
                <div class="card">
                    <div class="card-body-l p-0">
                        <div class="form-group pl-3 pt-3 pr-3">
                            <div class="d-flex w-100">
                                <Multiselect v-model="form.client" :options="clients" :taggable="false"
                                             :show-labels="false" tag-placeholder=""
                                             :placeholder="$t('common.client_placeholder')"
                                             class="form-control"
                                             @select="form.room = null"
                            ...
                            <div class="w-100 mt-2">
                                <Multiselect v-model="form.room" :options="occupiedRooms" :taggable="false"
                                             ...
                                             class="form-control"
                                             @select="form.client = null"
                                             label="room_name" track-by="id" style="min-width: 80%"></Multiselect>
                            </div>
                        </div>
                            ...
                            <div class="form-group col-md-6 col-lg-6">
                                ...
                                <div class="input-group">
                                    <input
                                            ...
                                            step="any"
                                            min="1"
                                            :max="form.discountType == 1 ? 100 : foodItemSubTotal"
                                            class="form-control"
                ...
                <div class="row no-print">
                    <div class="col-12 col-lg-6 mb-1">
                        <button
                                ...
                                @click="saveOrder($event, false)"
                                @keydown="form.onKeydown($event)"
                                :disabled="!form.room || selectedItemList.length <= 0"
                        >
            ...
            <div>
                <div v-if="currentProduct?.image" style="width: 100%; height: 17rem">
                    <img
                            ...
                            :src="currentProduct?.image?.replace('public/', '/storage/')"
                            alt="product image"
                ...
                <div id="invoice-POS">
                    <div style="max-width: 400px; margin: 0px auto">
                        <div class="info">
                            ...
                            <div class="text-bold text-center text-lg" v-show="hotel.hotel_name"
                            > {{ hotel.hotel_name }} <br
                            /></div>
                            <p>
                                <span
                                ...
                                /></span>
                                <span v-show="hotel.hotel_address"
                                >{{ $t("common.address") }} : {{ hotel.hotel_address }} <br
                                /></span>
                                <span v-show="hotel.hotel_email"
                                >{{ $t("common.phone") }} : {{ hotel.hotel_phone }} <br
                                /></span>
                                <span v-show="hotel.hotel_phone"
                                >{{ $t("common.email") }} : {{ hotel.hotel_email }} <br
                                /></span>
                                <span v-show="allData?.client?.name"
                                >{{ $t("common.client") }} : {{ allData.client.name }} <br
                                /></span>
                            </p>
                        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            selectedItemLists(){
                console.log('computed',this.selectedItemList);
        ...
        //   },
          async getOrderData(){
            ...
            this.get_order_data = data.data;
            this.hotel = data.data.hotel;
            this.form.client = data.data.customer;
            this.form.discount = data.data.items.discount;
            
            this.get_order_data.items.items.forEach((value) => {
                const addonNames = value.addOn?.map(add => add.item_name);
                const addonString = addonNames ? addonNames.join(' + ') : '';
                this.selectedItemList.push({
                        name: `${value?.name} (${value?.variant_name})`,
                        id: value.item_id,
                        variant: value?.variant,
                        addon: value.addOn,
                        addonAmount: (value.addOn.length) ? (data.data.items.addon / value.quantity) : 0,
                        addonString:addonString,
                        quantity: value.quantity,
                        total: parseFloat(value.total || 0) + (value.addOn.length) ? data.data.items.addon : 0
                    })
            });
          },
          ...
          exclusiveTaxAmount(amount, taxRate) {
            ...
            async saveOrder(event, isPayment = '')
            {
                ...
                await this.form
                    .post(window.location.origin + "/api/restaurant/order/update/" + this.$route.params.slug)
                    .then(({ data }) => {
                        this.resetForm();
                        toast.fire({ type: "success", title: "Order Updated successfully" });
                    })
                    .catch((error) => {
                        console.log(error)
            ...
            async saveInvoice(isDirect = true) {
                await this.form
                    .post(window.location.origin + "/api/food/order/invoice/update/" + this.$route.params.slug)
                    .then(({ data }) => {
