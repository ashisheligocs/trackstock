<template>
  <div class="mb-50">
            ...
            <div id="printMe" class="table-responsive table-custom mt-3">
              <table class="table">
                ...
                <tbody>
                  ...
                  <tr v-if="restaurantOrders && restaurantOrders?.length" v-for="(data, i) in restaurantOrders"
                    :key="i">
                    <template
                      v-if="!selectedType || (data.customer && selectedType?.id == 1) || (data.room && selectedType?.id == 2)">
                      <td>
                        <span>{{ i + 1 }}</span>
                      </td>
        ...
        <div id="invoice-POS">
          <div style="max-width: 400px; margin: 0px auto">
            <div class="info">
              ...
              <div class="text-bold text-center text-lg" v-show="allData.hotel.hotel_name"> {{ allData.hotel.hotel_name
                }}
                <br />
              </div>
              <p>
                ...
                <span v-show="allData.hotel.hotel_address">{{ $t("common.address") }} : {{ allData.hotel.hotel_address
                  }}
                  <br /></span>
                <span v-show="allData.hotel.hotel_email">{{ $t("common.phone") }} : {{ allData.hotel.hotel_phone }}
                  <br /></span>
                <span v-show="allData.hotel.hotel_phone">{{ $t("common.email") }} : {{ allData.hotel.hotel_email }}
                  <br /></span>
                <span v-show="allData?.customer?.name">{{ $t("common.client") }} : {{ allData.customer?.name }}
                  <br /></span>
              </p>
            </div>
            ...
            <div id="legalcopy" class="ml-2 mb-4">
              ...
              <div id="bar" style="overflow: hidden">
                <barcode width="2" height="25" fontSize="15" :value="allData.orderId">
    ...
    <Modal v-if="showSmallKotModal && allData" class="KOTPOS">
          ...
          <div style="max-width: 400px; margin: 0px auto">
            <div class="info">
              ...
              <p class="d-flex justify-content-between">
                ...
                <span v-show="allData?.customer?.name">{{ $t("common.client") }} : {{ allData.customer?.name }}
                  <br /></span>
                <span v-show="allData.orderId">Order No : {{ allData.orderId }} <br /></span>
              </p>
            </div>
      ...
      <div class="pos-modal-footer no-print flex" slot="modal-footer">
        <button @click="kotInvoice" class="modal-default-button btn btn-info">
          {{ $t("common.print") }}
        </button>

        <button class="modal-default-button btn btn-danger" @click="closeKOTModal">
          {{ $t("common.close") }}
        </button>
      </div>
    </Modal>
    ...
    <VModal v-if="showOrderStatusModal" v-model="showOrderStatusModal" @close="showOrderStatusModal = false">
      ...
      <div class="mt-1">
        <div class="form-group">
          ...
          <v-select v-model="order_status" :options="orderStatus" label="status_name"
            :class="{ 'is-invalid': payment_form.errors.has('account') }" name="account"
        ...
        <div>
          ...
          <div class="row" v-if="accounts">
            <div class="form-group col-md-6">
              ...
              <v-select v-model="payment_form.account" :options="accounts" label="ledgerName"
                :class="{ 'is-invalid': payment_form.errors.has('account') }" name="account"
                :placeholder="$t('common.account_placeholder')" />
            ...
            <div class="form-group col-md-6">
              ...
              <input ref="paidAmountInput" id="paidAmount" v-model="payment_form.paidAmount" type="number" step="any"
                class="form-control" :class="{ 'is-invalid': payment_form.errors.has('paidAmount') }" name="paidAmount"
                min="1" :max="payment_form.netTotal" :placeholder="$t('common.paid_amount_placeholder')" disabled />
          ...
          <div class="row">
            <div class="form-group col-md-6">
              ...
              <input id="receiptNo" v-model="payment_form.receiptNo" type="text" class="form-control"
                :class="{ 'is-invalid': payment_form.errors.has('receiptNo') }" name="receiptNo"
            ...
            <div class="form-group col-md-6">
              ...
              <input id="date" v-model="payment_form.date" type="date" class="form-control"
                :class="{ 'is-invalid': payment_form.errors.has('date') }" name="date" />
              <has-error :form="payment_form" field="date" />
            </div>
          </div>
          <div class="form-group">
            <label for="note">{{ $t("common.note") }}</label>
            <textarea id="note" v-model="payment_form.note" class="form-control"
              :class="{ 'is-invalid': payment_form.errors.has('note') }" :placeholder="$t('common.note_placeholder')" />
