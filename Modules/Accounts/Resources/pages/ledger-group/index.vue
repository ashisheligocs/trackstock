<template>
    <div class="mb-50">
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card" v-if="$can('ledger-group-list') ||
...
<!--                                <search v-model="query" @reset-pagination="resetPagination()" @reload="reload" />-->
                            ...
                            <div class="col-12 col-md-12 col-xl-4 text-right mb-2">
                                <div class="btn-group c-w-100">
                                    ...
                                    <router-link :to="{ name: 'ledger-group.create' }" class="btn btn-primary" v-if="$can('ledger-group-create')">
                                        {{ $t("common.create") }}
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr v-show="hotelCategoryItems.length" v-for="(data, i) in hotelCategoryItems" :key="i">
                                        ...
                                        <td>{{ data.position }}</td>
                                        <td v-if="$can('ledger-group-edit') ||
                                            $can('ledger-group-delete')
                                            " class="text-right no-print">
                                            <div class="btn-group">
                                                <router-link v-if="$can('ledger-group-edit')" v-tooltip="$t('common.edit')" :to="{
                                                    name: 'ledger-group.edit',
                                                    params: { slug: data.id },
                                                }" class="btn btn-info btn-sm">
                                                    <i class="fas fa-edit" />
                                                </router-link>
                                                <a v-if="$can('ledger-group-delete')" v-tooltip="$t('common.delete')" href="#"
                    ...
                    <div class="dtable-footer card-footer">
                        <div class="form-group row display-per-page m-0">
...
export default {
    ...
    metaInfo() {
        return { title: this.$t("ledger_group.index.page_title") };
    },
    components: {
        DateRangePicker,
    },
    data: () => ({
        breadcrumbsCurrent: "ledger_group.index.breadcrumbs_current",
        breadcrumbs: [
            {
                name: "ledger_group.index.breadcrumbs_first",
                url: "home",
            },
            {
                name: "ledger_group.index.breadcrumbs_second",
                url: "",
            },
        ],
        perPage: 10,
    ...
    methods: {
        ...
        async getData() {
            ...
            await this.$store.dispatch("operations/getHotelCategoryData", {
                path: "/api/account/ledger/group?page="+page+"&perPage=" + this.perPage,
            });
        },
                ...
                if (result.value) {
                    this.$store
                        .dispatch("operations/getDeleteData", {
                            path: "/api/account/ledger/group/delete/",
                            slug: slug,
                        })
                        .then((response) => {
                            if (response === true) {
