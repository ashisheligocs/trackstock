...
export default {
    ...
    methods: {
        // get category
        async getCategory() {
            const { data } = await this.form.get(window.location.origin + '/api/room/category/view/' + this.$route.params.slug)
            this.form.room_category_name = data.data.room_category_name
            this.form.room_cat_id = data.data.id
        },
        // update category
        async updateCategory() {
            await this.form
                .post(
                    ...
                    '/api/room/category/edit'
                )
                .then((response) => {
                    toast.fire({
                        type: 'success',
                        title: this.$t('rooms.categories.edit.success_msg'),
                    })
